class Admin::SalesReportsController < AdminController
  ...
  def sales_income_result
    if params[:start_cost_date] && params[:end_cost_date]
      ...
      @total_cost_of_goods = get_total_cost_of_goods
  ...
  def get_total_cost_of_goods
    OrderItem.joins(:item).select('COALESCE(SUM(order_items.item_quantity * items.cost), 0) AS cost_of_goods')[0].cost_of_goods
    # OrderItem.joins(:item).select('SUM(item_quantity * cost ) AS total_cost_of_goods')
  end
